...
1. Drop columns if you don't need them for the analysis.
2. Filter to keep specific columns if you need them for the analysis.

**Question 2: Handling Missing Data**
- What happens if you use `.dropna()` without specifying `subset`? How is this different from `.dropna(subset=['plannedShipDate', 'actualShipDate'])`?

If you use `.dropna()` without specifying `subset`, it will drop all rows with any missing values in any column.
If you use `.dropna(subset=['plannedShipDate', 'actualShipDate'])`, it will drop rows with missing values in only the `plannedShipDate` and `actualShipDate` columns. This is more selective and efficient.
- Why might you want to be selective about which columns to check for missing values? 
Data analysis is about focusing on the relevant columns and data. If you don't need to check for missing values in all columns, it's more efficient and easier to understand. Say each column is a feature and based on domain/context, you may not need to check for missing values in all columns. example if you are analyzing a dataset of customer orders, you may not need to check for missing values in the order created-by user column if you are only interested in the order amount.
